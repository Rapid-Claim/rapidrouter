...
| **Keys** | "who can spend what?" | virtual-key table: model and account scope, budget bar, limit status, last-used; create/rotate/revoke flows (secret shown once); per-key drill-down of spend and model mix ([virtual-keys.md](virtual-keys.md)) |
