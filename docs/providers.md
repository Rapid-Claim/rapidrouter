...
Two providers are backed by a *subscription* rather than a metered API key.
They are off unless configured, and carry caveats an operator must read
first — see [components/agent-subscriptions.md](components/agent-subscriptions.md).
...
| Codex | `codex_subscription` | Codex CLI `auth.json`, `file:`-referenced so a rotated token can be persisted | Private Responses endpoint; CLI header set with a configurable `Version` (a model gate); `max_tokens` cannot be carried; reasoning/verbosity floor configurable; a `prompt_cache_key` derived from the prefix (caller-overridable); attached PDFs rasterized to page images |
