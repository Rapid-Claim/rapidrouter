...
/// The same credential document with its refresh token neutralized, for
/// handing to a process that must use the account directly.
///
/// The gateway is the only writer allowed to rotate a credential. A borrower
/// holding a live refresh token could rotate it out from under us — killing
/// the account for every other holder, which is a fleet-wide outage caused by
/// one cooperative process doing something reasonable. So the field is
/// blanked rather than passed on, and a consumer that finds one still live is
/// entitled to refuse the credential.
///
/// `None` when the document is not JSON we recognise: better to lend nothing
/// than to lend something we could not prove we had defanged.
pub fn without_refresh_token(document: &str) -> Option<String> {
    let mut value: serde_json::Value = serde_json::from_str(document).ok()?;
    let mut cleared = false;
    for path in [
        vec!["tokens", "refresh_token"],
        vec!["refresh_token"],
        vec!["OPENAI_API_KEY_REFRESH"],
    ] {
        ...
        for (depth, key) in path.iter().enumerate() {
            ...
            if depth + 1 == path.len() {
                if map.contains_key(*key) {
                    map.insert((*key).to_owned(), serde_json::Value::String(String::new()));
                    cleared = true;
                }
                node = None;
            } else {
...
mod lend_tests {
    ...
    #[test]
    fn the_refresh_token_never_leaves_with_the_credential() {
        let doc = r#"{"auth_mode":"chatgpt","tokens":{"access_token":"at","refresh_token":"rt-live","id_token":"it"}}"#;
        let lent = without_refresh_token(doc).expect("a json document is lendable");
        assert!(
            !lent.contains("rt-live"),
            "the refresh token followed the credential out: {lent}"
        );
        assert!(
            lent.contains("\"access_token\":\"at\""),
            "the usable half survives"
        );
        assert!(
            lent.contains("\"refresh_token\":\"\""),
            "the field stays, blanked"
        );
    }

    #[test]
    fn a_top_level_refresh_token_is_caught_too() {
        let lent =
            without_refresh_token(r#"{"access_token":"at","refresh_token":"rt-live"}"#).unwrap();
        assert!(!lent.contains("rt-live"));
    }

    #[test]
    fn a_document_we_cannot_parse_is_not_lent_at_all() {
        assert!(without_refresh_token("sk-ant-oat01-an-opaque-token").is_none());
        assert!(without_refresh_token("").is_none());
    }
}
